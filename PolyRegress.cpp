...
const int MAXSIZE = 3;
double poly_leastsqr(double x[], double y[], int num_poly, double* ptr_A0, double*
	ptr_A1, double* ptr_A2);
void gauss_pivot(double x[][MAXSIZE], double y[], double* ptr_a0, double* ptr_a1,
	double* ptr_a2);
int main()
{
	double x[MAXPOINTS], y[MAXPOINTS];
	int i;
	int n_poly = 2; // degree of polynomial
	double ptr_A0, ptr_A1, ptr_A2;
	double err_Y, Sr, Sxy, r2;
	x[0] = 1.2; y[0] = 4.5;
	x[1] = 1.5; y[1] = 5.1;
	x[2] = 1.8; y[2] = 5.8;
	x[3] = 2.6; y[3] = 6.7;
	...
	// x, y, n_poly are the inputs to the function
	// ptr_A0, ptr_A1, ptr_A2 are the outputs from the functions using pointer
	// err_Y is the output from the function
	...
	cout << "y = " << ptr_A0 << " + " << ptr_A1 << "*x + " << ptr_A2 << "*x*x" <<
		endl << endl;
	...
	cout << "Error y = " << err_Y << endl;
	cout << "Sr = " << Sr << endl;
	// Compute the standard error
	Sxy = sqrt(Sr / ((double)MAXPOINTS - ((double)n_poly + 1)));
	cout << "The standard error = " << Sxy << endl;
	// Compute the coefficient of determination
	r2 = (err_Y - Sr) / err_Y;
	cout << "The coefficient of determination, r2 = " << r2 << endl << endl;
	return 0;
}
	...
	ptr_A1, double* ptr_A2)
{
	...
	sumx = 0.0; sumx2 = 0.0; sumx3 = 0.0; sumx4 = 0.0;
	...
	// X, Y are the inputs to the function
	// ptr_a0, ptr_a1, ptr_a2 are the outputs from the functions
	...
	// Now we want to pass ptr_a0, ptr_a1, ptr_a2 to the MAIN function
	// Passing to pointer
	*ptr_A0 = ptr_a0;
	*ptr_A1 = ptr_a1;
	*ptr_A2 = ptr_a2;
	...
	double* ptr_a2)
{
	...
	for (k = 0; k < MAXSIZE; ++k)
	{
		...
		// Test for singularity. If the pivot element is smaller than eps.
	...
	// Backsubstitution
	a[MAXSIZE - 1] = y[MAXSIZE - 1] / x[MAXSIZE - 1][MAXSIZE - 1];
